...
// Date:     25/09/2019                                                       *
// Descr:    Module for playing drum sounds                                   *
// ****************************************************************************

/*
 * Using modified code from: Michael Smith <michael@hurts.ca>
 * 
 * We modified his code so that it would allow us to play multiple
 * drum sounds rather than just a single sound.
 * 
 * His explanation is below for how the sound is play: *
...
ISR(TIMER1_COMPA_vect)
{
  ...
  {
    OCR2A = pgm_read_byte(&data[currentSample]);
  }
  currentSample++;
}
...
void setupPlayback(void)
{
  ...
  // Set timer to be the midpoint value of PWM.
  OCR2A = 128;

  // Set up Timer 1 to send a sample every interrupt.
  cli();

  // Enable interrupt when TCNT1 == OCR1A (p.136) Play per sample interrupt
  TIMSK1 |= _BV(OCIE1A);
...
void startPlayback(int16_t drum)
{
  ...
  maxSample = currentSample + sizes[drum];
  //Serial.println("Current: " + String(currentSample) + "\t Max: " + String(maxSample));
  ...
  // Initial value to be first value
  OCR2A = pgm_read_byte(&data[currentSample]);
